...
public class ImageExtractor extends AbstractExtractor {
    ...
    public void extractAbsentImagesOnly() {
        Path outputFolderPath = Paths.get("src/main/resources", getFetchConfig().getOutFolder());
        File[] files = outputFolderPath.toFile().listFiles();
        Set<String> allImagesToFetch = new HashSet<>(FileHelper.readWordNamesFromFile(getWordsTxtLocation()));
    ...
    private Runnable getImage(String word, int cnt) {
        return () -> {
            try {
                ...
                Path outputFolderPath = Paths.get("src/main/resources", getFetchConfig().getOutFolder(), word);
